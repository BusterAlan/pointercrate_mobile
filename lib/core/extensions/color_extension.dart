...
extension ColorExtension on Color {
  ...
  String toHex({bool leadingHashSign = true, bool alphaRadix = true}) =>
      '${leadingHashSign ? '#' : ''}'
      '${alphaRadix ? (a * 255.0).round().clamp(0, 255).toRadixString(16).padLeft(2, '0') : ''}'
  ...
  Color brighten([int amount = 10]) {
    ...
    final Color color = Color.fromARGB(
      ...
      math.max(0, math.min(255, (r * 255.0).round().clamp(0, 255) - operations)),
      math.max(0, math.min(255, (g * 255.0).round().clamp(0, 255) - operations)),
      math.max(0, math.min(255, (b * 255.0).round().clamp(0, 255) - operations)),
    );
